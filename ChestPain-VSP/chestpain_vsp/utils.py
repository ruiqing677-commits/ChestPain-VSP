...
def get_llm_client(api_key: Optional[str] = None, base_url: Optional[str] = None):
    """Create a configurable LLM API client.

    The public repository keeps the model backend configurable through
    environment variables so different provider endpoints can be used without
    ...
    load_env_file()
    resolved_key = api_key or os.environ.get("CHESTPAIN_VSP_API_KEY")
    resolved_base_url = base_url or os.environ.get("CHESTPAIN_VSP_BASE_URL")
    if not resolved_key:
        raise RuntimeError("CHESTPAIN_VSP_API_KEY is required.")
